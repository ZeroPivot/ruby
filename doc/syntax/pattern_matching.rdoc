...
Pattern matching is an experimental feature allowing deep matching of structured values, with checking the structure and binding matched parts to local variables.
...
One of the things developer should be aware about, which probably to be fixed in upcoming versions, is that pattern matching statement rewrites mentioned local variables on partial match, <i>even if the whole pattern is not matched</i>.
